    ...
    for file in &files {
        ...
        match check_conditions(file, db, conditions, tags) {
            ...
            Ok(false) => {
                ...
                let _skip_reason = SkipReason::ConditionNotMet;
...
pub fn rename_tag(
    ...
    for file in &files {
        ...
        let Some(current_tags) = db.get_tags(file)? else {
            let _skip_reason = SkipReason::Other;
            summary.add_skip();
            continue;
        };
...
/// Parsed tag mapping entry (old -> new)
...
/// Provide a helpful hint by actually attempting to parse with other formats.
/// This avoids heuristics and prefers concrete parser success.
fn format_mismatch_hint_parsed(content: &str, attempted: BatchFormat, delimiter: char) -> Option<String> {
    // Try JSON
    let is_json = serde_json::from_str::<serde_json::Value>(content).is_ok();
    // Try CSV: consider parsable if we can read at least one non-empty record
    let is_csv = {
        ...
        for rec in rdr.records() {
            match rec {
                ...
                Err(_) => {
                    // if first record already errors, treat as not CSV
                    break;
                }
            }
        }
    ...
    // Try PlainText: at least one line with >=2 whitespace-separated tokens
    let is_plain = content.lines().any(|line| {
        let t = line.trim();
        !t.is_empty() && !t.starts_with('#') && t.split_whitespace().count() >= 2
    });

    match attempted {
        BatchFormat::Json if is_csv => Some("Hint: The file appears to be CSV. Use '--format csv' (and '--delimiter' if needed).".to_string()),
        BatchFormat::Json if is_plain => Some("Hint: The file appears to be plain text. Use '--format text'.".to_string()),
        BatchFormat::Csv(_) if is_json => Some("Hint: The file may be JSON. Use '--format json'.".to_string()),
        BatchFormat::PlainText if is_json => Some("Hint: The file may be JSON. Use '--format json'.".to_string()),
        BatchFormat::PlainText if is_csv => Some("Hint: The file appears to be CSV. Use '--format csv'.".to_string()),
        _ => None,
    }
}

/// Apply tags from an input file to multiple files
#[allow(clippy::too_many_arguments)]
pub fn batch_from_file(
    ...
    let entries = match format {
        ...
        BatchFormat::Csv(delim) => parse_csv(&content, delim)?,
    ...
    if dry_run {
        ...
        for (i, e) in entries.iter().enumerate().take(10) {
            println!("  {}. {} <- [{}]", i + 1, e.file.display(), e.tags.join(", "));
        }
        if entries.len() > 10 {
    ...
    for entry in entries {
        if entry.tags.is_empty() {
            let _skip_reason = SkipReason::AlreadyExists;
            summary.add_skip();
            continue;
        }
...
// ===================== Tag Mapping (Bulk Rename Multiple Tags) =====================

/// Apply multiple tag rename mappings sourced from an input file.
/// Each mapping renames one tag (from) to another (to), merging if the target already exists.
pub fn bulk_map_tags(
    ...
    let mappings = match format {
        ...
        BatchFormat::Csv(delim) => parse_mapping_csv(&content, delim)?,
    ...
    for mapping in mappings {
        ...
        for file in files {
            ...
            if !tags.iter().any(|t| t == &mapping.from) {
                // May have changed due to previous mapping merging
                summary.add_skip();
                continue;
            }

            // Replace 'from' with 'to'; if 'to' already exists just remove 'from'
            ...
            // Remove empty placeholders and deduplicate
            ...
            let pair = Pair { file: file.clone(), tags: new_tags };
            match db.insert_pair(&pair) {
                Ok(()) => {
                    summary.add_success();
                    if !quiet {
                        println!("✓ '{}' → '{}' in {}", mapping.from, mapping.to, file.display());
                    }
                }
                Err(e) => {
                    summary.add_error(format!("{}: {}", file.display(), e));
                    if !quiet {
                        eprintln!("✗ Failed '{}' → '{}' in {}: {}", mapping.from, mapping.to, file.display(), e);
                    }
                }
            }
        }
...
fn parse_mapping_text(content: &str) -> Result<Vec<TagMapping>> {
    ...
    for (i, line) in content.lines().enumerate() {
        ...
        mappings.push(TagMapping { from: parts[0].to_string(), to: parts[1].to_string() });
    }
    Ok(mappings)
}

fn parse_mapping_csv(content: &str, delimiter: char) -> Result<Vec<TagMapping>> {
    ...
    for (i, rec) in rdr.records().enumerate() {
        let record = rec.map_err(|e| {
            ...
            match format_mismatch_hint_parsed(content, BatchFormat::Csv(delimiter), delimiter) {
                Some(hint) => TagrError::InvalidInput(format!("{}\n{}", base, hint)),
        ...
        mappings.push(TagMapping { from: from.to_string(), to: to.to_string() });
    }
    Ok(mappings)
}

fn parse_mapping_json(content: &str) -> Result<Vec<TagMapping>> {
    #[derive(serde::Deserialize)]
    struct JsonMap { from: String, to: String }
    ...
    Ok(parsed.into_iter().map(|jm| TagMapping { from: jm.from, to: jm.to }).collect())
}


fn parse_plaintext(content: &str) -> Result<Vec<BatchEntry>> {
...
fn parse_csv(content: &str, delimiter: char) -> Result<Vec<BatchEntry>> {
    // If the content looks like JSON, don't attempt CSV; return a hint.
    let trimmed = content.trim_start();
    if trimmed.starts_with('[') || trimmed.starts_with('{') {
    ...
    for (i, result) in rdr.records().enumerate() {
        ...
        // Remaining fields are tags; support a single field that itself is a comma-separated list
        for field in record.iter().enumerate().skip(1).map(|(_, f)| f) {
            let f = field.trim();
            if f.is_empty() {
                continue;
            }
            // If delimiter is not ',', allow inner comma-separated list in one field
            if f.contains(',') {
                tags.extend(f.split(',').map(str::trim).filter(|s| !s.is_empty()).map(|s| s.to_string()));
            } else {
...
fn parse_json(content: &str) -> Result<Vec<BatchEntry>> {
    #[derive(serde::Deserialize)]
    struct JsonEntry {
        file: String,
        tags: Vec<String>,
...
mod tests {
    ...
    fn test_parse_csv_ok_basic_and_quoted() {
        ...
        assert_eq!(entries[0].tags, vec!["tag1", "tag2"]);
        assert_eq!(entries[1].file, PathBuf::from("/c/d.md"));
        // quoted second field includes comma-separated tags
    ...
    #[test]
    fn test_parse_csv_bad_with_json_hint() {
        let input = r#"[{"file":"/a/b.txt","tags":["t1"]}]"#;
        let err = parse_csv(input, ',').unwrap_err();
        let msg = format!("{}", err);
        assert!(msg.contains("Invalid CSV"));
        assert!(msg.contains("Hint: The file may be JSON"));
    }
    ...
    #[test]
    fn test_parse_mapping_text_ok() {
        let input = "old1 new1\n# comment\nold2 new2";
        let maps = parse_mapping_text(input).unwrap();
        assert_eq!(maps.len(), 2);
        assert_eq!(maps[0], TagMapping { from: "old1".into(), to: "new1".into() });
    }

    #[test]
    fn test_parse_mapping_text_bad() {
        let input = "onlyone"; // missing second token
        let err = parse_mapping_text(input).unwrap_err();
        assert!(format!("{}", err).contains("Invalid mapping"));
    }

    #[test]
    fn test_parse_mapping_csv_ok() {
        let input = "old,new\nfoo,bar";
        let maps = parse_mapping_csv(input, ',').unwrap();
        assert_eq!(maps.len(), 2);
        assert_eq!(maps[1], TagMapping { from: "foo".into(), to: "bar".into() });
    }

    #[test]
    fn test_parse_mapping_csv_bad_record_len() {
        let input = "old,new,extra"; // 3 fields invalid
        let err = parse_mapping_csv(input, ',').unwrap_err();
        assert!(format!("{}", err).contains("expected exactly 2"));
    }

    #[test]
    fn test_parse_mapping_json_ok() {
        let input = r#"[{"from":"a","to":"b"},{"from":"c","to":"d"}]"#;
        let maps = parse_mapping_json(input).unwrap();
        assert_eq!(maps.len(), 2);
        assert_eq!(maps[0], TagMapping { from: "a".into(), to: "b".into() });
    }

    #[test]
    fn test_parse_mapping_json_bad_with_hint() {
        let input = "old,new"; // CSV-looking
        let err = parse_mapping_json(input).unwrap_err();
        let msg = format!("{}", err);
        assert!(msg.contains("Invalid JSON mapping"));
    ...
    fn test_bulk_map_tags_basic() {
        ...
        db.add_tags(f.path(), vec!["old".into(), "keep".into()]).unwrap();
    ...
    #[test]
    fn test_bulk_map_tags_merge_existing() {
        let test_db = TestDb::new("test_bulk_map_tags_merge_existing");
        let db = test_db.db();
        db.clear().unwrap();
        let f = TempFile::create("file.txt").unwrap();
        db.add_tags(f.path(), vec!["old".into(), "target".into(), "x".into()]).unwrap();
        let mapping_file = TempFile::create_with_content("map.txt", b"old target").unwrap();
        bulk_map_tags(
            db,
            mapping_file.path(),
            BatchFormat::PlainText,
        ...
        let tags = db.get_tags(f.path()).unwrap().unwrap();
        assert!(tags.contains(&"target".into()));
        assert!(!tags.contains(&"old".into()));
        assert!(tags.contains(&"x".into()));
        // Ensure no duplicates
        let mut seen = std::collections::HashSet::new();
        for t in tags { seen.insert(t); }
        assert_eq!(seen.len(), 2); // target, x
    }
}
